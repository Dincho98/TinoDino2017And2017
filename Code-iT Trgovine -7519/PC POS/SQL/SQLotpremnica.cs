...
namespace PCPOS.SQL
{
    ...
    {
        public static string InsertStavke(DataTable DT, bool otpremnicaNaSkladiste = false)
        {
            if (classSQL.remoteConnectionString == "")
            {
                ...
                try
                {
                    nonqueryCommand.CommandText = "INSERT  INTO otpremnica_stavke (kolicina,vpc,nbc,porez,broj_otpremnice,rabat,id_skladiste,sifra_robe,oduzmi,godina_otpremnice)" +
                        " VALUES (@kolicina,@vpc,@nbc,@porez,@broj_otpremnice,@rabat,@id_skladiste,@sifra_robe,@oduzmi,@godina_otpremnice,@id_otpremnice, @naplaceno_fakturom)";
                    ...
                    nonqueryCommand.Parameters.Add("@id_otpremnice", SqlDbType.NVarChar, 10);
                    nonqueryCommand.Parameters.Add("@naplaceno_fakturom", SqlDbType.Bit, 10);
                    ...
                    for (int i = 0; i < DT.Rows.Count; i++)
                    {
                        ...
                        nonqueryCommand.Parameters["@id_otpremnice"].Value = DT.Rows[i]["id_otpremnice"].ToString();
                        nonqueryCommand.Parameters["@naplaceno_fakturom"].Value = '0';
            ...
            else
            {
                ...
                try
                {
                    nonqueryCommand.CommandText = "INSERT  INTO otpremnica_stavke (kolicina,vpc,porez,broj_otpremnice,rabat,id_skladiste,sifra_robe,oduzmi,godina_otpremnice)" +
                        " VALUES (@kolicina,@vpc,@porez,@broj_otpremnice,@rabat,@id_skladiste,@sifra_robe,@oduzmi,@godina_otpremnice,@id_otpremnice,@naplaceno_fakturom)";

                    for (int i = 0; i < DT.Rows.Count; i++)
                    {
                        string sql = "INSERT INTO otpremnica_stavke (kolicina,vpc,nbc,porez,broj_otpremnice,rabat,id_skladiste,sifra_robe,oduzmi,godina_otpremnice, id_otpremnice, naplaceno_fakturom)" +
                        " VALUES (" +
                        "'" + DT.Rows[i]["kolicina"].ToString() + "'," +
                        "'" + DT.Rows[i]["vpc"].ToString().Replace(",", ".") + "'," +
                        "'" + DT.Rows[i]["nbc"].ToString() + "'," +
                        "'" + DT.Rows[i]["porez"].ToString() + "'," +
                        "'" + DT.Rows[i]["broj_otpremnice"].ToString() + "'," +
                        ...
                        "'" + DT.Rows[i]["id_otpremnice"].ToString() + "'," +
                        "'0' " +
                        ")";

                        if (otpremnicaNaSkladiste)
                            sql = @"INSERT INTO otpremnice_na_skl_stavke(
            id_otpremnice, broj_otpremnice, id_skladiste, sifra_robe,
            kolicina, nbc, vpc, porez, rabat, oduzmi, godina_otpremnice,
            porez_potrosnja, naplaceno_fakturom)
    VALUES ( " + DT.Rows[i]["id_otpremnice"].ToString() + @", " + DT.Rows[i]["broj_otpremnice"].ToString() + @", " + DT.Rows[i]["id_skladiste"].ToString() + @", '" + DT.Rows[i]["sifra_robe"].ToString() + @"',
            " + DT.Rows[i]["kolicina"].ToString().Replace(",", ".") + @", " + DT.Rows[i]["nbc"].ToString().Replace(",", ".") + @", " + DT.Rows[i]["vpc"].ToString().Replace(",", ".") + @", " + DT.Rows[i]["porez"].ToString().Replace(",", ".") + @", " + DT.Rows[i]["rabat"].ToString().Replace(",", ".") + @", " + (DT.Rows[i]["oduzmi"].ToString() == "DA" ? true : false).ToString().ToLower() + @", " + DT.Rows[i]["godina_otpremnice"].ToString() + @",
            0, false);";
        ...
        public static string UpdateStavke(DataTable DT)
        {
            if (classSQL.remoteConnectionString == "")
            {
                ...
                try
                {
                    nonqueryCommand.CommandText = "UPDATE otpremnica_stavke SET kolicina=@kolicina,vpc=@vpc,nbc=@nbc,porez=@porez,broj_otpremnice=@broj_otpremnice,rabat=@rabat,id_skladiste=@id_skladiste,sifra_robe=@sifra_robe,oduzmi=@oduzmi,godina_otpremnice=@godina_otpremnice WHERE id_stavka=@id_stavka";
                    nonqueryCommand.Parameters.Add("@kolicina", SqlDbType.NVarChar, 15);
                    nonqueryCommand.Parameters.Add("@vpc", SqlDbType.Money, 8);
                    nonqueryCommand.Parameters.Add("@nbc", SqlDbType.Money, 8);
                    nonqueryCommand.Parameters.Add("@porez", SqlDbType.NVarChar, 10);
                    ...
                    nonqueryCommand.Parameters.Add("@rabat", SqlDbType.NVarChar, 20);
                    nonqueryCommand.Parameters.Add("@id_skladiste", SqlDbType.Int, 4);
                    nonqueryCommand.Parameters.Add("@staro_skladiste", SqlDbType.Int, 4);
                    nonqueryCommand.Parameters.Add("@sifra_robe", SqlDbType.NVarChar, 20);
                    nonqueryCommand.Parameters.Add("@oduzmi", SqlDbType.NVarChar, 2);
                    nonqueryCommand.Parameters.Add("@godina_otpremnice", SqlDbType.NVarChar, 6);
                    ...
                    for (int i = 0; i < DT.Rows.Count; i++)
                    {
                        ...
                        nonqueryCommand.Parameters["@rabat"].Value = DT.Rows[i]["rabat"].ToString();
                        nonqueryCommand.Parameters["@id_skladiste"].Value = DT.Rows[i]["id_skladiste"].ToString();
                        nonqueryCommand.Parameters["@staro_skladiste"].Value = DT.Rows[i]["staro_skladiste"].ToString();
                        nonqueryCommand.Parameters["@sifra_robe"].Value = DT.Rows[i]["sifra_robe"].ToString();
                        nonqueryCommand.Parameters["@oduzmi"].Value = DT.Rows[i]["oduzmi"].ToString();
                        nonqueryCommand.Parameters["@godina_otpremnice"].Value = DT.Rows[i]["godina_otpremnice"].ToString();
