...
namespace PCPOS.SQL
{
    ...
    {
        public static string InsertStavke(DataTable DT, bool faktura)
        {
            if (classSQL.remoteConnectionString == "")
            {
                if (classSQL.connection.State == ConnectionState.Closed) { classSQL.connection.Open(); }
                SqlCeCommand nonqueryCommand = classSQL.connection.CreateCommand();
                try
                {
                    if (faktura)
                    {
                        nonqueryCommand.CommandText = "INSERT INTO faktura_stavke (kolicina,vpc,nbc,naziv,porez,broj_fakture,rabat,id_skladiste,sifra,oduzmi)" +
                            " VALUES (@kolicina,@vpc,@nbc,@naziv,@porez,@broj_fakture,@rabat,@id_skladiste,@sifra,@oduzmi,@povratna_naknada)";
                        ...
                        nonqueryCommand.Parameters.Add("@naziv", SqlDbType.Text, 18);
                    ...
                    {
                        nonqueryCommand.CommandText = "INSERT INTO faktura_van_stavke (kolicina, vpc, nbc, porez, broj_fakture, rabat, id_skladiste, sifra, oduzmi)" +
            ...
            else
            {
                ...
                try
                {
                    for (int i = 0; i < DT.Rows.Count; i++)
                    {
                        ...
                        if (faktura)
                        {
                            sql = "INSERT INTO faktura_stavke (kolicina, vpc, nbc, naziv, porez, broj_fakture, rabat, id_skladiste, sifra, " +
                                "povratna_naknada, oduzmi, povratna_naknada_izn, rabat_izn, mpc_rabat, ukupno_rabat, " +
                                "ukupno_vpc, ukupno_mpc, ukupno_mpc_rabat, ukupno_porez, id_kasa, id_ducan, ukupno_osnovica, ppmv) VALUES " +
                                "(" +
                        ...
                        {
                            sql = "INSERT INTO faktura_van_stavke (kolicina, vpc, porez, broj_fakture, rabat, id_skladiste, sifra, " +
                                "oduzmi, nbc, porez_potrosnja, povratna_naknada, rabat_izn, mpc_rabat, ukupno_rabat, " +
                                "ukupno_vpc, ukupno_mpc, ukupno_mpc_rabat, povratna_naknada_izn, ukupno_porez, ukupno_osnovica) VALUES " +
                                "(" +
                                "'" + DT.Rows[i]["kolicina"].ToString() + "'," +
                                "'" + DT.Rows[i]["vpc"].ToString().Replace(",", ".") + "'," +
                                "'" + DT.Rows[i]["porez"].ToString() + "'," +
        ...
        public static string UpdateStavke(DataTable DT, bool faktura)
        {
            if (classSQL.remoteConnectionString == "")
            {
                if (classSQL.connection.State == ConnectionState.Closed) { classSQL.connection.Open(); }
                ...
                try
                {
                    if (faktura)
                    {
                        nonqueryCommand.CommandText = "UPDATE faktura_stavke SET kolicina=@kolicina, vpc=@vpc, nbc=@nbc, porez=@porez, broj_fakture=@broj_fakture, rabat=@rabat, id_skladiste=@id_skladiste, sifra=@sifra, oduzmi=@oduzmi, povratna_naknada=@povratna_naknada, naziv=@naziv WHERE sifra=@sifra AND broj_fakture=@broj_fakture";
                    ...
                    {
                        nonqueryCommand.CommandText = "UPDATE faktura_van_stavke SET kolicina=@kolicina, vpc=@vpc, porez=@porez, broj_fakture=@broj_fakture, rabat=@rabat, id_skladiste=@id_skladiste, sifra=@sifra, oduzmi=@oduzmi, nbc=@nbc, povratna_naknada=@povratna_naknada WHERE sifra=@sifra AND broj_fakture=@broj_fakture";
                    ...
                    for (int i = 0; i < DT.Rows.Count; i++)
                    {
                        ...
                        if (faktura)
                        {
                            sql = "UPDATE faktura_stavke SET " +
                                ...
                                "id_kasa='" + DT.Rows[i]["id_kasa"].ToString() + "'," +
                                "id_ducan='" + DT.Rows[i]["id_ducan"].ToString() + "'," +
                        ...
                        {
                            sql = "UPDATE faktura_van_stavke SET " +
                                " kolicina='" + DT.Rows[i]["kolicina"].ToString() + "'," +
                                " vpc='" + DT.Rows[i]["vpc"].ToString().Replace(",", ".") + "'," +
                                " porez='" + DT.Rows[i]["porez"].ToString() + "'," +
                                " broj_fakture='" + DT.Rows[i]["broj_fakture"].ToString() + "'," +
                                ...
                                " ukupno_vpc='" + DT.Rows[i]["ukupno_vpc"].ToString().Replace(",", ".") + "'," +
                                " ukupno_mpc='" + DT.Rows[i]["ukupno_mpc"].ToString().Replace(",", ".") + "'," +
                                " ukupno_mpc_rabat='" + DT.Rows[i]["ukupno_mpc_rabat"].ToString().Replace(",", ".") + "'," + " povratna_naknada_izn='" + DT.Rows[i]["povratna_naknada_izn"].ToString().Replace(",", ".") + "'," +
                                " ukupno_porez='" + DT.Rows[i]["ukupno_porez"].ToString().Replace(",", ".") + "'," +
                                " ukupno_osnovica='" + DT.Rows[i]["ukupno_osnovica"].ToString().Replace(",", ".") + "'" +
                                //"id_kasa='" + DT.Rows[i]["id_kasa"].ToString() + "'," +
                                //"id_ducan='" + DT.Rows[i]["id_ducan"].ToString() + "'," +
