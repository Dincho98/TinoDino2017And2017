...
namespace PCPOS.Kasa
{
    public partial class frmPregledSmjene : Form
    {
        ...
        private void LoadData()
        {
            ...
            " WHERE datum_racuna>'" + Convert.ToDateTime(datumOD).ToString("yyyy-MM-dd H:mm:ss") + "' AND datum_racuna<'" + Convert.ToDateTime(datumDO).ToString("yyyy-MM-dd H:mm:ss") + "'" +
                "";
            ...
            if (DT.Rows.Count > 0 && DT.Rows[0][0].ToString() != "")
            {
                ...
                lblProdanoGotovina.Text = "Ukupno prodano u smjeni novčanice: " + string.Format("{0:0.00}", Convert.ToDouble(DT.Rows[0]["gotovina"].ToString())) + " kn";
                ...
                if (DT.Rows[0]["virman"].ToString() != "") { lblProdanoVirman.Text = "Ukupno prodano u smjeni transakcijski račun: " + string.Format("{0:0.00}", Convert.ToDouble(DT.Rows[0]["virman"].ToString())) + " kn"; } else { lblProdanoVirman.Text = "Transakcijski račun: 0.00 kn"; }
            }
            else
            {
                lblUkupno.Text = "Ukupno: 0.00 kn";
                lblProdanoGotovina.Text = "Novčanice: 0.00 kn";
                lblProdanoKartice.Text = "Kartice: 0.00 kn";
                lblProdanoVirman.Text = "Transakcijski račun: 0.00 kn";
            }
            ...
            foreach (DataRow row in DTblagajnici.Rows)
            {
                ...
                string imeBlagajnik = row["ime"].ToString() + " " + row["prezime"].ToString();
                ...
                lbl.Text = row["ime"].ToString() + " " + row["prezime"].ToString() + "  " + Convert.ToDouble(Convert.ToDouble(row["gotovina"].ToString()) + Convert.ToDouble(row["kartice"].ToString()) + Convert.ToDouble(a) + Convert.ToDouble(b)).ToString("#0.00") +
                    " kn\r\nNovčanice: " + Convert.ToDouble(row["gotovina"].ToString()).ToString("#0.00") +
                    " kn,  Kartice: " + Convert.ToDouble(row["kartice"].ToString()).ToString("#0.00") + " kn" +
                    " \n\rTransakcijski račun: " + Convert.ToDouble(b).ToString("#0.00") + " kn";
        ...
        private void PrometProdajneRobe(bool pice, bool hrana, bool stavke)
        {
            ...
            string sql = "SELECT " +
                ...
                " WHERE  racuni.datum_racuna>'" + Convert.ToDateTime(datumOD).ToString("yyyy-MM-dd H:mm:ss") + "'" +
                " AND racuni.datum_racuna<'" + Convert.ToDateTime(datumDO).ToString("yyyy-MM-dd H:mm:ss") + "'" +
            ...
            {
                PrintTextLine(new string('-', RecLineChars));
                PrintTextLine("UKUPNO NOVČANICE:  " + UG.ToString("#0.00"));
            ...
            {
                PrintTextLine(new string('-', RecLineChars));
                PrintTextLine("UKUPNO TRANSAKCIJSKI RAČUN:  " + UV.ToString("#0.00"));
                //for (int i = 0; i < DTosnovica.Rows.Count; i++)
                //{
        ...
        {
            if (DTsetting.Rows[0]["posPrinterBool"].ToString() != "1")
            {
                MessageBox.Show("Mali printer nije omogućen u postavkama za pos opremu", "Upozorenje!");
                return;
            }
