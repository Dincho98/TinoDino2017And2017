...
{
    public partial class frmPromjenaCijeneBezSkladista : Form
    {
        public frmPromjenaCijeneBezSkladista()
        {
            InitializeComponent();
        }
        ...
        public string porezNovi { get; set; }
        ...
        {
            //DS skladiste
            DS_Skladiste = classSQL.select("SELECT * FROM skladiste ORDER BY skladiste", "skladiste").Tables[0];
        ...
        private void txtMpc_Leave(object sender, EventArgs e)
        {
            ...
            {
                txtVpc.Text = Convert.ToDouble(Convert.ToDouble(txtMpc.Text) / (1 + Convert.ToDouble(porez) / 100)).ToString("#0.0000");
            }
            else
            {
        ...
        private void btnPromjena_Click(object sender, EventArgs e)
        {
            ...
            this.mpc = txtMpc.Text;
            this.vpc = txtVpc.Text;

            double m = Convert.ToDouble(txtMpc.Text);
            double v = Convert.ToDouble(txtVpc.Text);
            double p = Math.Round((m / v - 1) * 100, 2);
            string a = p.ToString();
            this.porezNovi = p.ToString().Replace(".", ",");
