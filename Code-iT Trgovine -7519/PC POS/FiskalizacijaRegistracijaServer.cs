...
namespace PCPOS
{
    internal class FiskalizacijaRegistracijaServer
    {
        ...
        {
            public static DataSet SendDataOnServer(string web_adress, string oib, string tvrtka, string tel,
                string backup_baze, string blagajna, string ducan, string verzija, string robnoBool, string ukupno)
            {
                ...
                try
                {
                    if (Convert.ToDecimal(ukupno) > 100)
                    {
                        if (Util.CheckConnection.Check())
                        {
                            ...
                            string postData = "oib=" + oib + "&tvrtka=" + tvrtka + "&tel=" + tel + "&backup=" + backup_baze +
                                "&blagajna=" + blagajna + "&ukupno=" + ukupno + "&ducan=" + ducan + "&verzija=" + verzija +
                                "&robno=" + robnoBool;
