...
{
    public partial class frmFakturaVanStorno : Form
    {
        public frmFakturaVanStorno()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            DataTable DTzki = classSQL.select("Select zki, storno From fakture_van where broj_fakture = '" + textBox1.Text + "'", "zki").Tables[0];
            if (DTzki.Rows[0]["zki"].ToString() == "")
            {
                MessageBox.Show("Faktura nije fiskalizirana !");
                return;
            }
            ...
            {
                MessageBox.Show("Krivi broj fakture_van.", "Greška");
                return;
            }

            DataTable DT = classSQL.select("SELECT broj_fakture FROM fakture_van WHERE broj_fakture='" + broj + "'", "fakture_van").Tables[0];
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DTzki = classSQL.select("Select zki, storno From fakture_van where broj_fakture = '" + textBox1.Text + "'", "zki").Tables[0];
            }
            catch
            {
                return;
        ...
        {
            DataTable DSbr = classSQL.select("SELECT MAX(CAST(broj_fakture AS integer)) FROM fakture_van", "fakture_van").Tables[0];
        ...
        {
            DataTable DSracun = classSQL.select("SELECT storno FROM fakture_van where broj_fakture='" + broj + "'", "fakture_van").Tables[0];
        ...
        {
            DataTable DTracun = classSQL.select("SELECT * FROM fakture_van where broj_fakture='" + staraFaktura + "'", "fakture_van").Tables[0];
            ...
            string sql = "INSERT INTO fakture_van (broj_fakture,id_odrediste,id_fakturirati,date,dateDVO,datum_valute," +
            ...
            string sqlStorno = "UPDATE fakture_van SET storno='DA' WHERE broj_fakture='" + staraFaktura + "'";
        ...
        private void SrediStavkeRobuSkladiste(ref DataTable DSfakturaStavke, string staraFaktura, string novaFaktura, string oduzmi)
        {
            ...
            provjera_sql(SQL.SQLfaktura.InsertStavke(DSfakturaStavke, false));
        }

        private void StornirajFakturu(string staraFaktura)
        {
            //ubaci u racuni i vrati broj nove fakture_van, oduzmi, način plaćanja i zki
            ...
            DataTable DSfakturaStavke = classSQL.select("SELECT * FROM faktura_van_stavke where broj_fakture='" + staraFaktura + "'", "faktura_van_stavke").Tables[0];

            SrediStavkeRobuSkladiste(ref DSfakturaStavke, staraFaktura, novaFaktura, oduzmi);

            DataTable DT = classSQL.select("SELECT date, ukupno FROM fakture_van where broj_fakture='" + novaFaktura + "'", "fakture_van").Tables[0];
            ...
            if (DT.Rows.Count > 0)
            {
                if (zki != "")
                {
                    ...
                    provjera_sql(classSQL.insert("INSERT INTO aktivnost_zaposlenici (id_zaposlenik,datum,radnja) VALUES" +
                        ...
                        "'Storno fakture_van broj " + staraFaktura + " (fiskalizirana), nova faktura broj: " + novaFaktura + " (fiskalizirana)')"));
                }
                else
                {
                    provjera_sql(classSQL.insert("INSERT INTO aktivnost_zaposlenici (id_zaposlenik,datum,radnja) VALUES" +
                    "('" + Properties.Settings.Default.id_zaposlenik + "','" + DateTime.Now.ToString("yyyy-MM-dd H:mm:ss") + "'," +
                    "'Storno fakture_van broj " + staraFaktura + " (nije fiskalizirana), nova faktura broj: " + novaFaktura + " (nije fiskalizirana)')"));
                }
            }
            else
            {
                MessageBox.Show("Greška kod fiskalizacije storna fakture_van!", "Greška!");
                provjera_sql(classSQL.insert("INSERT INTO aktivnost_zaposlenici (id_zaposlenik,datum,radnja) VALUES" +
                    "('" + Properties.Settings.Default.id_zaposlenik + "','" + DateTime.Now.ToString("yyyy-MM-dd H:mm:ss") + "'," +
                    "'Storno fakture_van broj " + staraFaktura + " (fiskalizirana), nova faktura broj: " + novaFaktura + " (nije fiskalizirana)')"));
            }
        }
        ...
        private void FiskalizirajFakturu(DataTable DTsend, DateTime datum, int brojFakture, string ukupno, string nacinPlacanja)
        {
            ...
            try
            {
                fiskalizacija = Fiskalizacija.classFiskalizacija.Fiskalizacija(
                    ...
                    iznos_marza,
                    DTnaknade,
                    Convert.ToDecimal(ukupno),//.ToString().Replace(",", ".")
                    nacinPlacanja,
                    false,
                    "faktura"
                    );
        ...
        private void frmFakturaStorno_Load(object sender, EventArgs e)
        {
            ...
            //{
            id_kasa = Class.Postavke.id_maloprodaja_naplatni_uredaj.ToString();
