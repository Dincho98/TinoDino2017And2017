...
{
    public partial class repFakturaNovo : Form
    {
        public repFakturaNovo()
        {
            InitializeComponent();
        }
        ...
        public int id_partner_poslovnica { get; set; }
        ...
        {
            p17 = new ReportParameter("povratna_naknada", "0".ToString());
            ReportParameter p11 = new ReportParameter("ppNaziv", "");
            ReportParameter p12 = new ReportParameter("ppAdresa", "");
            ReportParameter p13 = new ReportParameter("ppGrad", "");
            ReportParameter p14 = new ReportParameter("ppDrzava", "");
            ReportParameter p15 = new ReportParameter("ppPartnerPoslovnice", id_partner_poslovnica.ToString());
...
from partner_poslovnice pp
left join zemlja d on pp.id_drzava = d.id_zemlja
left join grad g on pp.id_grad = g.id_grad
where id_partner_poslovnica = (select partner_poslovnica from fakture where broj_fakture = '{0}' and id_ducan = {1} and id_kasa = {2});", broj_dokumenta, poslovnica, naplatni);

                if (id_partner_poslovnica > 0)
                {
                    sql = string.Format(@"select pp.id_partner_poslovnica, pp.naziv, pp.adresa, d.zemlja, g.posta || ' ' || g.grad as grad
from partner_poslovnice pp
left join zemlja d on pp.id_drzava = d.id_zemlja
left join grad g on pp.id_grad = g.id_grad
where id_partner_poslovnica = {0};", id_partner_poslovnica);
                }

                DataSet dsPartnerPoslovnica = classSQL.select(sql, "partner_poslovnice");
                if (dsPartnerPoslovnica != null && dsPartnerPoslovnica.Tables.Count > 0 && dsPartnerPoslovnica.Tables[0] != null && dsPartnerPoslovnica.Tables[0].Rows.Count > 0)
                {
                    DataRow drRow = dsPartnerPoslovnica.Tables[0].Rows[0];

                    id_partner_poslovnica = Convert.ToInt32(drRow["id_partner_poslovnica"].ToString());
                    p11 = new ReportParameter("ppNaziv", drRow["naziv"].ToString());
                    p12 = new ReportParameter("ppAdresa", drRow["adresa"].ToString());
                    p13 = new ReportParameter("ppGrad", drRow["grad"].ToString());
                    p14 = new ReportParameter("ppDrzava", drRow["zemlja"].ToString());
                    p15 = new ReportParameter("ppPartnerPoslovnice", id_partner_poslovnica.ToString());
                }

                //this.reportViewer1.LocalReport.SetParameters(new ReportParameter[] { p11, p12, p13, p14, p15 });
            }
...
left join zemlja d on pp.id_drzava = d.id_zemlja
left join grad g on pp.id_grad = g.id_grad
where id_partner_poslovnica = (select partner_poslovnica from otpremnice where broj_otpremnice = '{0}' and id_skladiste = {1});", broj_dokumenta, from_skladiste);
...
where id_partner_poslovnica = {0};", id_partner_poslovnica);
            ...
            if (dokumenat == "FAK")
            {
                string valut = " Select fakture.id_valuta, valute.ime_valute AS valuta FROM fakture " +
                " LEFT JOIN valute ON valute.id_valuta=fakture.id_valuta WHERE fakture.broj_fakture='" + broj_dokumenta + "' AND fakture.id_ducan='" + poslovnica + "' AND fakture.id_kasa='" + naplatni + "'";
                ...
                string sql = "SELECT valute.naziv, ime_valute,fakture.tecaj,fakture.ukupno FROM fakture,valute WHERE broj_fakture= '" + broj_dokumenta + "'" +
                " AND fakture.id_valuta=valute.id_valuta AND fakture.id_ducan='" + poslovnica + "' AND fakture.id_kasa='" + naplatni + "'";
            ...
            else if (dokumenat == "PON")
            {
                ...
                string sql = "SELECT valute.naziv, ime_valute,ponude.tecaj,ponude.ukupno FROM ponude,valute WHERE broj_ponude= '" + broj_dokumenta + "'" +
            ...
            this.reportViewer1.LocalReport.SetParameters(new ReportParameter[] { p1, p2, p3, p4, p5, p6, p8, p9, p10, p11, p12, p13, p14, p15, p17, p18, p19, p20 });
            ...
            if (DT.Rows.Count > 0)
            {
                ...
                if (DT.Rows[0]["naziv"].ToString() != "")
                {
                    if (valuta == " kn")
                        ...
                        return "Prema tečaju " + tecaj.ToString() + " ukupna vrijednost iznosi " + Math.Round(ukupno, 3).ToString("#0.00") + " KN";
                }
                else
                {
                    return " ";
        ...
        private void FillRacun(string broj, string[] imeTablica, string poslovnica, string naplatni)
        {
            ...
            string sql2 = "SELECT " +
                ...
                " CASE WHEN length(racun_stavke.naziv) IS NULL OR racun_stavke.naziv = '' THEN roba.naziv ELSE racun_stavke.naziv END as naziv," +
                " roba.jm as jm," +
            ...
            for (int i = 0; i < DT.Rows.Count; i++)
            {
                ...
                StopePDVa(Math.Round((stopa * 100), 0), Math.Round(pdvStavka, 3), Math.Round(((osnovicaStavka)), 3));
            }
            ...
            //ReportParameter p10 = new ReportParameter("sakrij_jedinicnu_cijenu", sakrij_jedinicnu_cijenu_0_1);
        ...
        private void FillFaktura(string broj, string[] imeTablica)
        {
            ...
            else
            {
                for (int i = 0; i < DT.Rows.Count; i++)
                {
                    ...
                    porez = Math.Round(Convert.ToDecimal(DT.Rows[i]["porez"].ToString()), 3);
                    rabat = Math.Round(Convert.ToDecimal(DT.Rows[i]["rabat"].ToString()), 3);
                    povratna_naknada = Math.Round(Convert.ToDecimal(DT.Rows[i]["povratna_naknada"].ToString()), 3);

                    if (povratna_naknada != 0)
                    {
                        /*if (!samoIspis)
                        {
                            mpc = (vpc * (1 + porez / 100)) - (povratna_naknada / kol);
                            vpc = mpc / (1 + porez / 100);
                            vpc_cisto = vpc;
                        }*/
                        UKPpovratna_naknada += povratna_naknada;
                    }
            ...
            if (UKPpovratna_naknada != 0)
            {
                sveUkupno += UKPpovratna_naknada;
                p17 = new ReportParameter("povratna_naknada", UKPpovratna_naknada.ToString());
            }

            string val = " Select fakture.id_valuta, valute.ime_valute AS valuta FROM fakture " +
             " LEFT JOIN valute ON valute.id_valuta=fakture.id_valuta WHERE fakture.broj_fakture='" + broj + "' AND fakture.id_ducan='" + poslovnica + "' AND fakture.id_kasa='" + naplatni + "'";
            ...
            string sqlPoslovnicaPartner = "select COALESCE(pp.naziv||, 0) AS partner_poslovnica from partners_poslovnice pp LEFT JOIN faktura f on pp.id_partner_poslovnica = f.partner_poslovnica where f.broj_fakture = '" + broj + "';";
            ...
            string sql = "SELECT " +
                ...
                " LEFT JOIN zemlja ON zemlja.id_zemlja=partners.id_zemlja" +
                //" LEFT JOIN partner_poslovnice pp ON " + imeTablica[0] + ".partner_poslovnica = pp.id_partner_poslovnica" +
                //" LEFT JOIN zemlja zem ON pp." +
...
imeTablica[0], broj, poslovnica, naplatni);
        ...
        private void FillPonude(string broj, string[] imeTablica, string imeBrojPonude, string imeGodinaPonude)
        {
            ...
            for (int i = 0; i < DT.Rows.Count; i++)
            {
                ...
                DT.Rows[i].SetField("rabat", Math.Round(rabat, 3));
                DT.Rows[i].SetField("kolicina", Math.Round(kol, 3));
                DT.Rows[i].SetField("mpcStavka", Math.Round(mpcStavka + pdvStavka, 3));
                DT.Rows[i].SetField("rabatStavka", Math.Round(rabatStavka, 3));
