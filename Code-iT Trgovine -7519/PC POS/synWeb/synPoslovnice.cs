...
namespace PCPOS.synWeb
{
    internal class synPoslovnice
    {
        ...
        public void Send()
        {
            ...
            if (posalji_sve)
                if (MessageBox.Show("Želite poslati poslovnice?", "Sinkronizacija", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                {
                    return;
                }

            string query = "";
            query = "SELECT * FROM ducan WHERE (editirano='1' or novo='1');";
            ...
            foreach (DataRow r in DT.Rows)
            {
                ...
                {
                    tempDel = "DELETE FROM poslovnice WHERE oib='" + DTpodaci_tvrtka.Rows[0]["oib"].ToString() + "' AND poslovnica='" + poslovnica + "';~";
