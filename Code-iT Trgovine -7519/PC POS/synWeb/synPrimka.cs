...
namespace PCPOS.synWeb
{
    internal class synPrimka
    {
        ...
        public void Send()
        {
            ...
            if (posalji_sve)
            {
                if (MessageBox.Show("Želite poslati primke?", "Sinkronizacija", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                {
                    return;
                }
                filter = "";
            ...
            {
                filter = "  WHERE (primka.editirano='1' OR primka.novo='1')";
            }
            //****************************************************************************

            string query = @"SELECT
            '0'::numeric marza,
            '0'::numeric as iznos_marze,
            vpc as prodajna_cijena,
            mpc as prodajna_cijena_sa_porezom,
            primka.broj,
            primka.id_skladiste,
            primka.originalni_dokument as ulazni_dok,
            primka.id_partner,
            '0'::numeric tecaj,
            primka.datum,
            primka.id_izradio as id_zaposlenik,
            ...
            primka.novo,
            primka.editirano,
            REPLACE(primka_stavke.kolicina,',','.')::numeric as kolicina,
            REPLACE(primka_stavke.kolicina,',','.')::numeric as u_pakiranju,
            '1'::numeric as broj_paketa,
            primka_stavke.nbc as cijena_po_komadu,
            REPLACE(primka_stavke.rabat,',','.')::numeric as rabat,
            ROUND((primka_stavke.nbc-(primka_stavke.nbc*REPLACE(primka_stavke.rabat,',','.')::numeric/100)),4) as nabavna_cijena,
            REPLACE(pdv,',','.')::numeric as ulazni_porez,
            ROUND((primka_stavke.nbc-(primka_stavke.nbc*REPLACE(primka_stavke.rabat,',','.')::numeric/100))*REPLACE(primka_stavke.kolicina,',','.')::numeric,4) as nabavni_iznos,
            '0'::numeric as povratna_naknada
            FROM primka
            LEFT JOIN primka_stavke ON primka.id_primka=primka_stavke.id_primka
            LEFT JOIN zaposlenici ON primka.id_izradio=zaposlenici.id_zaposlenik" + filter;
            ...
            foreach (DataRow r in DT.Rows)
            {
                ...
                if (posalji_sve)
                {
                    tempDel = "DELETE FROM primke WHERE oib='" + DTpodaci_tvrtka.Rows[0]["oib"].ToString() + "' " +
                            " AND poslovnica='" + poslovnica + "' AND YEAR(datum)='" + datum.Year.ToString() + "'" +
                            " AND is_kalkulacija='0';~";
                }
                else
                {
                    ...
                    {
                        tempDel = "DELETE FROM primke WHERE oib='" + DTpodaci_tvrtka.Rows[0]["oib"].ToString() + "' " +
                            " AND poslovnica='" + poslovnica + "' AND broj='" + r["broj"].ToString() + "'" +
        ...
        public void UzmiPodatkeSaWeba()
        {
            ...
            string sql = "SELECT * FROM primke" +
                " WHERE (editirano='1') AND OIB='" + DTpodaci_tvrtka.Rows[0]["oib"].ToString() + "' AND poslovnica='" + poslovnica + "'";
        ...
        private void SpremiHeader(DataRow r, DataTable DT)
        {
            ...
            classSQL.delete("DELETE FROM primka WHERE broj_primke='" + r["broj"].ToString() + "' AND is_kalkulacija='" + r["is_kalkulacija"].ToString() + "';" +
                "DELETE FROM primka_stavke WHERE broj_primke='" + r["broj"].ToString() + "' AND is_kalkulacija='" + r["is_kalkulacija"].ToString() + "';");
            ...
            string sql = "INSERT INTO primka (" +
                ...
                "napomena,is_kalkulacija,iznos,id_poslovnica,novo,editirano)" +
                " VALUES " +
                "(" +
                "'" + r["broj"].ToString() + "'," +
