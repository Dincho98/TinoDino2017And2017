...
namespace PCPOS.synWeb
{
    internal class synZaposlenici
    {
        ...
        public void Send()
        {
            ...
            if (posalji_sve)
            {
                if (MessageBox.Show("Želite poslati zaposlenike?", "Sinkronizacija", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                {
                    return;
                }
            ...
            foreach (DataRow r in DT.Rows)
            {
                ...
                sql += "INSERT INTO djelatnik (id_zaposlenik, ime, prezime, id_grad, adresa," +
                    "id_dopustenje,oib_zaposlenika,tel,datum_rodenja,email,mob,aktivan,zaporka,kartica,oib,poslovnica,editirano,novo,datum_syn) VALUES (" +
            ...
            if (sql.Length > 4)
            {
                ...
                {
                    MessageBox.Show("Greška sa sinkronizacijom zaposlenika.\r\nProvijerite dali postoje dva zaposlenika sa istim oib-om.");
                }
            }
        }
                    ...
                    foreach (DataRow r in DT.Rows)
                    {
                        ...
                        query = "BEGIN; " +
                            "DELETE FROM zaposlenici WHERE id_zaposlenik='" + r["id_zaposlenik"].ToString() + "';" +
                                ...
                                " id_zaposlenik,ime,prezime,id_grad,adresa,id_dopustenje,oib,tel,datum_rodenja,email,mob,aktivan,zaporka,novo,editirano" +
                                ") VALUES (" +
                                " '" + r["id_zaposlenik"].ToString() + "'," +
                                " '" + r["ime"].ToString() + "'," +
                                " '" + r["prezime"].ToString() + "'," +
